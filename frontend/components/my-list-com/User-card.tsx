...
function UserPopup({ isOpen, onClose, user, isOnline }: any) {
  ...
  return (
    <ModalWrapper>
      <div className="fixed inset-0 bg-neutral-950/40 dark:bg-neutral-950/60 flex items-center justify-center z-50 p-4 backdrop-blur-xl">
        <div className="bg-white dark:bg-zinc-900 border border-neutral-100 dark:border-zinc-800 rounded-2xl p-6 w-full max-w-sm shadow-xl relative animate-slideUp">
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="relative">
              ...
              <span className={`absolute -bottom-1 -right-1 flex h-3.5 w-3.5 rounded-full border-2 border-white dark:border-zinc-900 ${isOnline ? "bg-emerald-500" : "bg-neutral-400 dark:bg-zinc-650"
                }`}>
                {isOnline && (
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${isOnline
      ...
      >
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span className={`absolute -bottom-1 -right-1 flex h-3 w-3 rounded-full border-2 border-white dark:border-zinc-900 ${isOnline ? "bg-emerald-500" : "bg-neutral-400 dark:bg-zinc-650"
              }`}>
              {isOnline && (
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              )}
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${isOnline
                ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-900/30"
                : "bg-neutral-100 dark:bg-zinc-800 text-neutral-500 dark:text-zinc-400 border border-neutral-200/50 dark:border-zinc-700/50"
                }`}>
                {isOnline ? "Online" : "Offline"}
